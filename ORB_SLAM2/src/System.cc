...
bool System::LoadMap(const string &filename)
{
    ...
    in.close();
    cout << "test2 !!!!!!!!!!!!!!!!!!\n";
    ofstream ft("MapPoint.txt");
    if (ft.fail())  {
        cout << "Load MapPoint.txt Fail!!!!!!!!!\n";
    }
    const vector<MapPoint*> &vpMPs = mpMap->GetAllMapPoints();
    ft << "Size = " << vpMPs.size() << endl;
    for(size_t i=0, iend=vpMPs.size(); i<iend;i++)
    {
        if(vpMPs[i]->isBad() )
            continue;
        cv::Mat pos = vpMPs[i]->GetWorldPos();
        ft << setprecision(5) << pos.at<float>(0) << " " << pos.at<float>(1) << " " << pos.at<float>(2) << endl;
    }
    ft.close();
    cout << "MapPoint.txt Save Successfully!\n";
    return true;
}
